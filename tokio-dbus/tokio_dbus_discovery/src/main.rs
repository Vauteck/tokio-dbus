...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let iface_token = cr.register("com.example.dbustest", |b| {
        ...
        /* Nouvelle methode wouhou !! :-) */
        b.method_with_cr_async("HelloThere", ("name","byte_array"), ("reply",), |mut ctx, cr, (name, byte_array): (String, Vec<u8>)| {
            let hello: &mut Hello = cr.data_mut(ctx.path()).unwrap(); // ok_or_else(|| MethodErr::no_path(ctx.path()))?;
                                                                      // And here's what happens when the method is called.
            println!("Called HelloThere function from {}!", name);
            hello.called_count += 1;
            let s = format!("Hello {}! This API has been used {} times.", name, hello.called_count);
            async move {
                // Let's wait half a second just to show off how async we are.
                ...
                let byte_iterator = byte_array.iter();
                for (idx, val) in byte_iterator.enumerate() {
                    println!("byte_array {} = {:#02x}", idx, val);
                }

                ctx.reply(Ok((s,)))  // The reply is sent when ctx is dropped / goes out of scope.
            }
        });
    });
